...
This code is adapted from https://github.com/JonathonLuiten/TrackEval/blob/master/scripts/run_tao.py
"""
...
parser.add_argument("--category_info", type=str, default="", help="path to category info folder")
parser.add_argument("--GT_FOLDER", type=str, default="results/gt", help="path to gt data")
parser.add_argument("--TRACKERS_FOLDER", type=str, default="results/predictions", help="path to prediction folder (in this folder, there should be 'tracker.json', e.g., 'bytetrack.json')")
parser.add_argument("--OUTPUT_FOLDER", type=str, default="output", help="path to save eval results")
parser.add_argument("--TRACKERS_TO_EVAL", nargs='+', default=['bytetrack'], help="Filenames of trackers to eval (if None, all in folder) ['bytetrack', 'deepsort', 'sort'], can also add other trackers")
...
if __name__ == '__main__':
    ...
    default_dataset_config = trackeval.datasets.SEMTRACK.get_default_dataset_config()
    default_metrics_config = {'METRICS': ['HOTA', 'STMAP', 'TrackMAP']} 
    ...
    dataset_list = [trackeval.datasets.SemTrackTR(dataset_config)]
    metrics_list = []

    for metric in [trackeval.metrics.STMAP, trackeval.metrics.TrackMAP, trackeval.metrics.HOTA]:
