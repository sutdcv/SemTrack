    ...
    def get_preprocessed_seq_data(self, raw_data, cls):
        ...
        for t in range(raw_data['num_timesteps']):
            if raw_data['is_main_gt'][t] is None:
                raw_data['is_main_gt'][t] = np.array([], dtype=np.int)
            if raw_data['rel_sub_class'][t] is None:
                raw_data['rel_sub_class'][t] = np.array([[]], dtype=np.int)
            if  raw_data['rel_obj_class'][t] is None:
                raw_data['rel_obj_class'][t] = np.array([[]], dtype=np.int)
            if raw_data['is_main_track'][t] is None:
                raw_data['is_main_track'][t] = np.array([], dtype=np.int)
            if raw_data['rel_sub_list'][t] is None:
                raw_data['rel_sub_list'][t] = np.array([[]], dtype=np.int)
            if raw_data['rel_obj_list'][t] is None:
                raw_data['rel_obj_list'][t] = np.array([[]], dtype=np.int)
